    ...
    def _music_download_text(self) -> str:
        ...
        if self.lang == "ja":
            return (
                ...
                "`/reload_yt_cookies` — cookie 再読込（Bot 運用者のみ）\n\n"
                "**ボイス**\n"
                "`/join` `/leave` — VC 接続・切断\n\n"
                "Now Playing: Pause / Skip / Stop（確認）/ Loop / QLoop / QShuffle。"
                "次曲があるときだけキュー表示（最大5曲＋ページング）。"
                "終了時は Queue Finished 等に差し替え、操作はグレーアウト（寄付リンクのみ有効）。\n\n"
                "**ダウンロード（PLANA）**\n"
        ...
        return (
            ...
            "**Queue**\n"
            "`/queue` `/nowplaying` `/shuffle` `/clear` `/remove` `/loop`\n"
            "`/reload_yt_cookies` — reload cookies (bot operators only)\n\n"
            "**Voice**\n"
            ...
            "When finished: Queue Finished (etc.), controls greyed out (donation link stays). \n\n"
            "**Download (PLANA)**\n"
    ...
    def _utilities_text(self) -> str:
        ...
        if self.lang == "ja":
            return (
                ...
                "• `/updates` — GitHub コミット履歴（Components V2・5件ページング）\n"
        ...
        return (
            ...
            "• `/updates` — GitHub commit history (Components V2, 5 per page)\n"
